...
//get by Id
export const getEnviosById = async(value)=>{
    const db = await connectDB();
    const collection = db.collection('envios');
...
//post
export const PostEnvios = async(value)=>{
    const db = await connectDB();
    const collection = db.collection('envios');
...
//Delete
export const deleteEnvios = async(HospedajesID)=>{
    console.log(HospedajesID);
    let db = await connectDB();
    let collection = db.collection('envios');
    let data = await collection.deleteOne({_id: new ObjectId(HospedajesID)});
    return data
}

//Update
export const actualizarEnvios= async(id,value)=>{
    let db = await connectDB();
    let collection = db.collection('envios');
    console.log(value);
    console.log(id);
